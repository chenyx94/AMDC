function decode(rec, myDate){
  ...
  while(j<rec.search(":")){
    while(flg){
      flg = 0; 
      while(rec[k]!="," && rec[k]!=":" && k<=rec.search(":")) k++;
      p = rec.substr(j+1,k-j-1);
      qCons="qAC,qAX,qAU,qAo,qAO,qAS,qAr,qAR,qAZ,qAI";
      if(qCons.indexOf(p)!=-1){
        //j = k;
        //k++;
        //while(rec[k]!="," && rec[k]!=":" && k<=rec.search(":")) k++;
        flg = 1;
      }
      Ifwide = rec.substr(j+1,4);
      if(Ifwide == "WIDE"){
        //j = k;
        //k++;
        //while(rec[k]!="," && rec[k]!=":" && k<=rec.search(":")) k++;
      ...
      IfTR = rec.substr(j+1,5);
      if(IfTR=="TRACE" || IfTR=="RELAY" ||IfTR=="TCPIP"){
        //j = k;
        //k++
        //while(rec[k]!="," && rec[k]!=":" && k<=rec.search(":")) k++;
        flg = 1;
    ...
    if(rec[k-1]=="*") k--;
    if(k<=rec.search(":")){
      Path.push(rec.substr(j+1,k-j-1));
    }
    if(rec[k+1]=="*") k++;
    j = k;
    k++;
